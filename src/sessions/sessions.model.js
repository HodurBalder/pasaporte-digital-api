...
schema.post('save', function(err, doc, next) {
    if(err) return next(new Messages(err).sessionSaveError)
    next()
})

schema.post('remove', function(err, doc, next) {
    if(err) return next(new Messages(err).sessionDeleteError)
    next()
})

schema.post('findOne', function(err, doc, next) {
    if(err) return next(new Messages(err).sessionGetError)
    next()
})
