const Model = require('./games.model.mysql')
...
async function createGame(data) {
    try {
        ...
        const exists = await Model.findOne({ where: { userId: data.userId, gameName: data.gameName } });

        if (exists)
            return exists

        const game = await Model.create(data)
